...
async function requestDoorUnlockAndLogAccess({ actionType, item, quantity = 1, projectName = 'Personal' }) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            addLog(actorId, 'Door Access Failed', `Door unlock denied during ${actionType}: ${quantity}x ${itemName} (${itemId}) in ${projectName}. HTTP ${response.status}${details ? ` - ${details}` : ''} via ${endpoint}.`);
            doorLinkHealthState.available = false;
            doorLinkHealthState.lastError = `Door unlock HTTP ${response.status}${details ? ` - ${details}` : ''}`.trim();
...
barcodeInput.addEventListener('input', () => {
    ...
    loginScanAutoSubmitTimer = setTimeout(async () => {
        loginScanAutoSubmitTimer = null;
