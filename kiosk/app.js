    ...
    if (!linkStatus.available) {
        ...
        if (endpoint) {
            try {
                ...
                addLog(actorId, 'Door Access Failed', `Direct door unlock failed during ${actionType}: ${quantity}x ${itemName} (${itemId}) in ${projectName}. HTTP ${response.status}${details ? ` - ${details}` : ''}. Falling back to queue.`);
            } catch (error) {
