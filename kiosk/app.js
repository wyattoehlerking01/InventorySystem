...
function renderDoorPage() {
    ...
    testUnlockBtn.onclick = async () => {
        await requestManualDoorUnlockAndLogAccess('kiosk door page test unlock');
        await refreshStatus();
    };
...
let loginScanAutoSubmitTimer = null;

barcodeInput.addEventListener('keydown', async (e) => {
    if (e.key === 'Enter' || e.key === 'NumpadEnter') {
        e.preventDefault();
        if (loginScanAutoSubmitTimer) {
            clearTimeout(loginScanAutoSubmitTimer);
            loginScanAutoSubmitTimer = null;
        }
...
function setUserPrivilegedPasswordHash(userId, hashValue) {
    ...
    if (currentUser && String(currentUser.id || '').trim().toUpperCase() === normalizedUserId) {
        ...
        currentUser.auth_password_hash = normalizedHash;
    }

    const target = mockUsers.find(u => String(u.id || '').trim().toUpperCase() === normalizedUserId);
    if (target) {
        target.privileged_password_hash = normalizedHash;
        target.auth_password_hash = normalizedHash;
    }
}
...
async function savePrivilegedPasswordHashForCurrentUser(hashValue) {
    ...
    const payloadCandidates = [
        { auth_password_hash: normalizedHash },
        { authentication_password_hash: normalizedHash },
...
function renderDebugSession(el) {
    ...
    el.innerHTML = `
        ...
        ${_dbgSection('Controls', `
            <div class="debug-actions">
                ...
                <button class="dbg-btn btn btn-secondary" id="dbg-door-open">
                    <i class="ph ph-door-open"></i> Open Door (3s)
                </button>
    ...
    document.getElementById('dbg-door-open')?.addEventListener('click', async () => {
        if (!userCanPerformPrivilegedActions()) {
            showToast('Only teacher/developer accounts can open the door from kiosk controls.', 'error');
            return;
        }
        await requestManualDoorUnlockAndLogAccess('kiosk debug session control');
    });
